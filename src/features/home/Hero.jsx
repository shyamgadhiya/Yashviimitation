...
import { IG, LOGO } from "../../lib/config.js";
import { ChevronRight, IgIcon, WaIcon } from "../../components/icons/index.jsx";
import { trackLead } from "../../lib/analytics.js";
import { BUSINESS_CITY, BUSINESS_WHATSAPP_LINK, getOccasionPath } from "../../lib/site.js";

export default function Hero({ total, newCount }) {
  return (
    <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", position: "relative", overflow: "hidden" }} className="hero-light">
      ...
      >
        <div className="hero-copy" style={{ animation: "fadeUp .5s ease both" }}>
          ...
          <p className="hero-description" style={{ fontSize: 15, color: "var(--text-muted)", lineHeight: 1.72, marginBottom: 32, maxWidth: 480, fontFamily: "'DM Sans',sans-serif" }}>
            Discover bridal, festive and daily wear imitation jewellery from {BUSINESS_CITY}. Explore kundan, polki and temple-inspired designs with quick WhatsApp support and pan-India delivery.
          </p>
          <div className="hero-buttons" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <Link to="/collection" className="btn-primary hero-button" style={{ padding: "13px 30px", fontSize: 13, fontWeight: 600, letterSpacing: ".1em", textTransform: "uppercase", borderRadius: 2, textDecoration: "none", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
              Explore Collection
            </Link>
            <a
              href={`${BUSINESS_WHATSAPP_LINK}?text=Hi! Please share your latest bridal and festive catalogue.`}
              target="_blank"
              rel="noopener"
              className="btn-outline hero-button"
              style={{ padding: "12px 24px", fontSize: 13, letterSpacing: ".08em", textTransform: "uppercase", borderRadius: 2, display: "inline-flex", alignItems: "center", gap: 6 }}
              onClick={() => trackLead("whatsapp", { placement: "hero_catalogue" })}
            >
              <WaIcon />
              View Catalogue
            </a>
        ...
        <div className="hero-side" style={{ display: "flex", flexDirection: "column", gap: 14, animation: "fadeUp .5s ease both .12s" }}>
          ...
          {[{ key: "festive", i: "🪔", n: "Festive", d: "Kundan & polki designs" }, { key: "wedding", i: "💒", n: "Bridal", d: "Complete wedding sets" }, { key: "daily", i: "🌸", n: "Daily Wear", d: "Elegant & lightweight" }].map((occasion, idx) => (
            <Link
